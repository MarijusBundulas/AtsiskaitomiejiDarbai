 ...
 */
export const getAttribute = (env, object, item, _arguments = new Map(), type = TwingTemplate.ANY_CALL, isDefinedTest = false, ignoreStrictCheck = false, sandboxed = false) => {
    let _do = () => {
        let message;
        ...
        if (type !== TwingTemplate.METHOD_CALL) {
            ...
            if (object) {
                if ((isMap(object) && object.has(arrayItem)) || (isPlainObject(object) && Reflect.has(object, arrayItem))) {
                    if (isDefinedTest) {
                        return true;
                    }
